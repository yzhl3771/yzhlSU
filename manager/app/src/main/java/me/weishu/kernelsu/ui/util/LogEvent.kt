...
fun getBugreportFile(context: Context): File {
    ...
    shell.newJob().add("tar -czf ${bootlogFile.absolutePath} -C /data/adb/yzhlsu/log .").exec()
    ...
    shell.newJob().add("du -sh /data/adb/yzhlsu/* > ${ksuFileSize.absolutePath}").exec()
    shell.newJob().add("cp /data/system/packages.list ${appListFile.absolutePath}").exec()
    shell.newJob().add("getprop > ${propFile.absolutePath}").exec()
    shell.newJob().add("cp /data/adb/yzhlsu/.allowlist ${allowListFile.absolutePath}").exec()
