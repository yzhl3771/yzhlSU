...
/// Find PIDs of processes running in the yzhlSU su domain (u:r:yzhlsu:s0).
    ...
    for entry in entries.flatten() {
        ...
        for fd_entry in fds.flatten() {
            ...
            if let Ok(target) = fs::read_link(&link_path) {
                ...
                if target_str.contains("[yzhlsu_driver") || target_str.contains("[yzhlsu_fdwrapper]") {
                    pids.push(pid);
                    break;
                }
            }
...
pub fn unload() -> Result<()> {
    info!("unload: starting yzhlSU unload sequence");
